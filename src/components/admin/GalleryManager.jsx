...
import Sidebar from './Sidebar'; // If Sidebar is not used, you can remove this import.
...
const GalleryManager = () => {
    ...
    const handleDeleteImage = async (imageId) => {
        setLoading(true);
        try {
            await deleteImage(selectedGallery.title, imageId);
            const updatedGallery = { ...selectedGallery };
            updatedGallery.images = updatedGallery.images.filter((img) => img._id !== imageId);
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            {/* Image Upload Form */}
            <form onSubmit={handleSubmit} className="mb-6 p-6 bg-white rounded-lg shadow-md space-y-4">
                <h2 className="text-2xl font-semibold text-gray-800">Upload New Images</h2>
                <input
                    type="text"
                    name="title"
            ...
            {selectedGallery && (
                <div className="mt-8">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {selectedGallery.images.map((image) => (
                            <div key={image._id} className="relative group">
                                <img
                                    src={`https://isa-matara-api-2005-batch.vercel.app${image.url}`}
                                    alt={image.title}
                                ...
                                <div className="absolute top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button
                                        className="bg-white px-4 py-2 rounded text-gray-800 hover:bg-gray-100 transition duration-200"
                    ...
                    <button
                        ...
                        onClick={() => handleDeleteGallery(selectedGallery.title)}
                    >
                        Delete Gallery
                    </button>
                </div>
